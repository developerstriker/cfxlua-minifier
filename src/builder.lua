...
local function build()
 ...
 for f in p:lines()do
  ...
  if rel and not ignored(rel) then
   ...
   if protected then local raw=rd(f);local d=output.."/"..rel;md(d:match("(.+)/[^/]+$")or output);wr(d,raw);stats.config=stats.config+1
   else
   ...
   if isLua then local raw=rd(f);local result=mini(raw);code[side][#code[side]+1]=result;stats.lua=stats.lua+1;stats.bytesIn=stats.bytesIn+#raw;stats.bytesOut=stats.bytesOut+#result else local raw=rd(f);local d=output.."/"..rel;md(d:match("(.+)/[^/]+$")or output);wr(d,raw);stats.other=stats.other+1 end
   end
  end
 end;p:close()
